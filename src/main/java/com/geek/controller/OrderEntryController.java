...
import com.geek.service.MenuManager;
...
public class OrderEntryController {
    ...
    public String showMenus(Model model,
        ...
        if (useCookie != null && useCookie.equalsIgnoreCase("on")) {
            int expire = 3600 * 24 * 30;// 如果使用 cookie，则将过期时间设为 1 个月。
            logger.info("用户选择使用 cookie，进入使用 cookies 的控制逻辑！");
            ...
            int expire = -1;// 如果使用 cookie，则将过期时间设为 -1 控制该 cookie 立刻过期。
            logger.info("用户没有选择使用 cookie，进入不使用 cookies 的控制逻辑！");
     ...
     * @return
     */
    @RequestMapping(value = "/delete/{mid}", method = RequestMethod.GET)// 按照 ID 展示。
    public String deleteMenu(Model model,
                             @PathVariable int mid,
                             HttpServletRequest request,
                             HttpServletResponse response) {
        menuManager.deleteMenuByMid(mid);// 删除对应 menu。
